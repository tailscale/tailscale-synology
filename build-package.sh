...
download_tailscale() {
  ...
  local pkg_name="tailscale_${VERSION}_${ARCH}.tgz"
...
make_spk() {
  local spk_tmp_dir=$1
  local spk_filename="tailscale_${VERSION}_${ARCH}.spk"
  ...
  # Generate INFO file
  ./src/INFO.sh ${VERSION} ${ARCH} ${pkg_size} >"${spk_tmp_dir}"/INFO
...
main() {
  echo "> Building package for VERSION=${VERSION} ARCH=${ARCH}"
  download_tailscale
  make_pkg
}
